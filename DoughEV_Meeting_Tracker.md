...
- [x] **Cryptographic Security:** Secured the dynamic QR coupon redemption workflow by implementing server-side HMAC-SHA256 signatures for payload verification. This strictly prevents token forgery and guarantees that only server-signed QR payloads are accepted by staff scanners.
- [x] **Map Reliability & Modularization:** Implemented a robust React Error Boundary around the 3D Map context to prevent WebGL/MapLibre runtime crashes from bringing down the entire application. Decomposed the monolithic `InteractiveMap` component (originally 1300+ lines) by extracting complex marker-rendering logic and core map initialization/interaction logic into specialized custom hooks (`useMapInit`, `useVenueMarkers`, etc.), drastically improving codebase maintainability.
- [x] **Mobile UX Polish:** Fixed critical UI performance issues on Android regarding the CSS marquee ticker components (`SafetyDashboard`, `SafetyTicker`). Replaced static percentage-based translations with responsive DOM-aware calculations (`scrollWidth` + `innerWidth`) to guarantee consistent, smooth, framerate-independent readability regardless of device width or dynamic payload sizes.
